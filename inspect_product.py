...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Inspect one public Partzilla product page.")
    parser.add_argument("--part-number", required=True, help="OEM part number to inspect from the CSV.")
...
def main() -> int:
    ...
    settings = ProbeSettings(headless=args.headless, slow_mo=args.slow_mo, timeout=args.timeout)
    ...
    try:
        ...
        LOGGER.exception("Product inspection failed for %s", record.oem_part_number)
    ...
    _write_observation(output_dir / "observation.json", observation)
    _write_diagnostics(output_dir / "diagnostics.txt", observation, exception_message)
    _print_summary(observation, output_dir / "observation.json")
    return 0 if observation.page_classification != PageClassification.NAVIGATION_ERROR else 1


def _write_observation(path: Path, observation: ProductObservation) -> None:
