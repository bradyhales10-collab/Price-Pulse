...
function returnToPriceCheckAfterJob(job) {
  ...
  window.setTimeout(() => {
    ...
    if (importBatchId) {
      window.location.href = `/imports?import_batch_id=${encodeURIComponent(importBatchId)}&view=results&message=${message}#recent-files`;
    } else {
      window.location.href = `/imports?message=${message}#recent-files`;
    }
  }, 2500);
}
